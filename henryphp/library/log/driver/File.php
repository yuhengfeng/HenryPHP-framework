...
class File
{
    ...
    public function __construct(App $app, $config = [])
    {
        ...
        if (empty($this->config['path'])) {
            $this->config['path'] = $this->app->getStoragePath() . 'log' . DIRECTORY_SEPARATOR;
    ...
    protected function getMasterLogFile()
    {
        if ($this->config['single']) {
            ...
            if ($this->config['max_files']) {
                ...
                $filename = date('Ym') . DIRECTORY_SEPARATOR . $this->app->getFrameName().'-'.date('d') . $cli . '.log';
            }
    ...
    protected function getApartLevelFile($path, $type)
    {
        ...
        if ($this->config['single']) {
            ...
            $name = date('Ymd') . '-' . $type . $cli;
        } else {
            $name = date('d') . '-' . $type . $cli;
        }

        return $path . DIRECTORY_SEPARATOR . $this->app->getFrameName().'-'.$name . '.log';
    }
